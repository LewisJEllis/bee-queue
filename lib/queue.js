...
function Queue(name, settings) {
  ...
  var reportReady = barrier(
    2 + this.settings.isWorker + this.settings.getEvents,
    this.emit.bind(this, 'ready')
  );

  var makeClient = function (clientName) {
    this.settings.reconnectOnError = function () {
      return true;
    };
    this[clientName] = redis.createClient(this.settings.redis);
    redis.Promise.onPossiblyUnhandledRejection(this.emit.bind(this, 'error'));
    this[clientName].on('ready', function () {
      reportReady();
    });
  }.bind(this);
...
Queue.prototype.process = function (concurrency, handler) {
  ...
  this.bclient.on('close', restartProcessing);
